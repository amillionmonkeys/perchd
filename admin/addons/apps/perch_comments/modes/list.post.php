<?php
    ...
    <ul class="smartbar">
        ...
        <li class="new <?php echo ($status=='pending'?'selected':''); ?>"><a href="<?php echo PerchUtil::html($API->app_path().'/'.'?status=pending'); ?>"><?php echo $Lang->get('Pending (%s)', $pending_comment_count); ?></a></li>
        <?php
...
?>
            <tr>
				<td class="primary">
					...
					<a href="<?php echo $HTML->encode($API->app_path()); ?>/edit/?id=<?php echo $HTML->encode(urlencode($Comment->id())); ?>" class="edit">
					<?php echo strftime('%d&nbsp;%h&nbsp;%y', strtotime($Comment->commentDateTime())); ?>
					</a>
				</td>
                <td><?php echo $HTML->encode($Comment->parentTitle()); ?></td>
				<td><?php echo $HTML->encode($Comment->commentName()); ?></td>
				
                <td><?php echo $HTML->encode($Comment->commentEmail()); ?></td>
                <td><?php echo $HTML->encode($Comment->commentScore()); ?></td>
